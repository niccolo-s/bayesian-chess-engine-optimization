...
abline(v=0, col="green3", lwd=2)

# according to both methods, LMR_slope = 0.3517588 is chosen, 
...
candidate_2 <- list(name="E", params=list(NMP_intercept=3, NMP_slope=0, LMR_intercept=1,
                                            LMR_slope=x_grid[which.max(pred$mean)], RFP_intercept=-30, RFP_slope=150))
...
df_2_results$LMR_slope <- x_grid[which.max(pred$mean)]

df_2_results <- rbind(df_results, df_2_results) ##  Run it just once!

x <- as.matrix(df_2_results$LMR_slope)
colnames(x) <- "LMR_slope"
...
# New Plot
y_min <- min(pred$mean - qnorm(0.025, lower.tail = F) * pred$sd, df_2_results$mean_rating)
y_max <- max(pred$mean + qnorm(0.025, lower.tail = F) * pred$sd, df_2_results$mean_rating)
...
# For step 3, according to EI, LMR_slope = 0.3517588 is chosen
...
df_3_results$LMR_slope <- x_grid[which.max(pred$mean)]

df_3_results <- rbind(df_2_results, df_3_results) ##  Run it just once!

x <- as.matrix(df_3_results$LMR_slope)
colnames(x) <- "LMR_slope"
...
# New Plot
y_min <- min(pred$mean - qnorm(0.025, lower.tail = F) * pred$sd, df_3_results$mean_rating)
y_max <- max(pred$mean + qnorm(0.025, lower.tail = F) * pred$sd, df_3_results$mean_rating)
...
# For step 4, according to EI, LMR_slope = 0.2311558 would be chosen 

save.image(file="environment_task6.RData")